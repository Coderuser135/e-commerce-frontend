  ...
  return (
    <div className='min-h-[90vh] h-full w-full p-3 sm:p-5 flex flex-col gap-2 mb-20 overflow-x-hidden'>
      ...
      <div className='w-full grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 py-2'>
        <SummaryCard
          onClick={() => nevigate('/adminPannel/all-products')}
          icon={<BsFillBoxFill className='text-[1.4rem] sm:text-[1.8rem] text-white' />}
          title='Total Products'
          value={productsStore.length} />
        <SummaryCard
          onClick={() => nevigate('/adminPannel/products-category')}
          icon={<FaFolderClosed className='text-[1.4rem] sm:text-[1.8rem] text-white' />}
          title='Total Category'
          value={productsCategory.length} />
        <SummaryCard
          onClick={() => nevigate('/adminPannel/create-products')}
          icon={<IoBagAddSharp className='text-[1.4rem] sm:text-[1.8rem] text-white' />}
          title='Create Products'
          value={productsStore.length} />
        <SummaryCard
          onClick={() => nevigate('/adminPannel/all-products')}
          icon={<FaEdit className='text-[1.4rem] sm:text-[1.8rem] text-white' />}
          title='Edit Products'
          value='' />
      </div>
...
const SummaryCard = ({ onClick, icon, title, value }) => (
  <div onClick={onClick} className='min-h-24 sm:h-25 w-full min-w-0 flex items-center justify-center gap-2 sm:gap-4 px-2 bg-[#e9e4e4] rounded-[10px] shadow-2xl cursor-pointer'>
    ...
    <div className='min-w-0 flex flex-col'>
      <h1 className='text-xs sm:text-[15px] font-serif truncate'>{title}</h1>
      <h1 className='text-lg sm:text-[1.4rem] font-medium'>{value}</h1>
    </div>
  </div>
)
...
export const DeletePopup = () => {
  ...
  const deleteProductsHandler = async () => {
    console.log("products is deleted")
  ...
  return <div className='h-screen w-full bg-black/35 flex justify-center items-center fixed inset-0 z-50 px-4'>
    ...
    <div className='w-full max-w-[30rem] min-h-48 rounded-2xl flex flex-col items-center p-4 justify-center gap-5 bg-[#252424]'>
      ...
      <div className='w-full max-w-80 flex flex-col sm:flex-row gap-2 sm:justify-between items-stretch sm:items-center'>
        ...
        <button onClick={deleteProductsHandler} className='w-full sm:w-auto px-12 py-2 bg-[#d61710] text-white rounded-xl'>{loading === true ? <ButtonLoading /> : "Ok"}</button>
      </div>
    </div>
  </div>
}
...
export const ProductsTitle = () => <div className='hidden sm:flex h-10 w-full items-center justify-between border border-solid border-[#e2dede] p-2 bg-[#d6d5d5] rounded-[5px]'>
  <div className='w-18'>
    <h1 className='font-serif text-base sm:text-[18px]'>Image</h1>
  </div>
...
export const Products = ({ details }) => {
  ...
  return <div className='w-full min-h-20 sm:h-14 border border-solid border-[#e6e3e3] flex flex-wrap sm:flex-nowrap justify-between items-center gap-2 p-2 font-serif rounded-lg bg-[#e4e0e0] shadow-2xl'>
    ...
    <div className='h-12 w-14 sm:w-18 shrink-0 rounded-[5px] bg-[#e9e4e4] flex justify-center items-center'>
      <img src={details.image} alt='' className='h-10 w-10 object-contain' />
    </div>
    <div
      className='flex justify-center items-center gap-0.5 w-auto sm:w-30'>
      <MdOutlineStar className='text-[#e9c46a] text-xl' />
      <h1>{details.rating}</h1>
    </div>
    <div className='flex-1 min-w-20 sm:w-30 sm:flex-none text-center truncate'>
    ...
    <div className='h-full w-full sm:w-45 flex justify-end sm:justify-between items-center sm:px-8'>
      <button onClick={() => { dispatch(setEditDataStore(details)); nevigate('/adminPannel/products-update') }} className='w-10 flex justify-center items-center text-xl text-[#356fec] font-bold p-2'><MdOutlineEdit />
      </button>
      <button onClick={() => dispatch(setDeletePopup(details))} className='w-10 flex justify-center items-center text-xl font-bold text-[#d41515] p-2'><MdOutlineDelete />
      </button>
    </div>
  </div>
}
